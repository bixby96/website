...
The following report intends to analyze avocado prices and sales volume from 2015 to early 2018 across the US.  The dataset comes from the Hass Avocado Board.  The corresponding code and plots are shown below.  
...
It looks like there could be a slight L-shaped curve for both types of avocados, but the linear model does an adequate job of describing the data. For simplicity, let's just fit a linear price-response function for all markets to compare their elasticities.

```{r}
out <- lapply(unique(avocados_market$region), function(b){
            df <- avocados_market %>%
              filter(region == b)
...
```{r echo = F}
p9 <- ggplot(out, aes(x=market, y=abs(elasticity))) +
